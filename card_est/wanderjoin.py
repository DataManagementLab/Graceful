...
class WanderJoin():
    ...
    def __init__(self, pg_kwargs: Dict = None, dd_kwargs: Dict = None,
                 verbose=False, cache_dir="./sql_cache", walks_timeout=0.5,
                 seed=1234, use_tries=True, trie_cache=None, max_succ_walks=100):
        if pg_kwargs is not None:
            self.con = pg.connect(**pg_kwargs)
            self.cursor = self.con.cursor()
        elif dd_kwargs is not None:
            assert os.path.exists(dd_kwargs["database"]), f"Database file does not exist {dd_kwargs['database']}"
            self.con = duckdb.connect(**dd_kwargs)
            self.cursor = self.con.cursor()
        self.walks_timeout = walks_timeout
        self.verbose = verbose
        self.use_tries = use_tries
        self.max_succ_walks = max_succ_walks
        if self.use_tries:
            tstart = time.time()
    ...
    def init_path_details(self, path, sg):
        ...
        if first_node not in self.init_sels:
            ...
            try:
                ...
                print(f'Tried to execute: {exec_sql}')
                raise e
            outputs = self.cursor.fetchall()
            self.init_sels[first_node] = outputs
            if len(outputs) == 0:
                raise EmptyResultException(f"no outputs for first node: {exec_sql}")
                print(f"no outputs for first node: {exec_sql}", flush=True)
    ...
    def get_counts(self, qrep):
        ...
        for node, info in join_graph.nodes(data=True):
            ...
            for edge in edges:
                ...
                join_cond = edge_data["join_condition"]

                if node == join_cond.table_name1:
                    sels.append(f'{join_cond.table_name1}.{join_cond.column_name1}')
                elif node == join_cond.table_name2:
                    sels.append(f'{join_cond.table_name2}.{join_cond.column_name2}')
                else:
                    raise ValueError(f"Node {node} not in join condition {join_cond}")

                join_graph[edge[0]][edge[1]][
                    join_cond.table_name1] = f'{join_cond.table_name1}.{join_cond.column_name1}'
                join_graph[edge[0]][edge[1]][
                    join_cond.table_name2] = f'{join_cond.table_name2}.{join_cond.column_name2}'
        ...
        for node in subset_keys:
            ...
            for i in range(MAX_WALKS):
                ...
                elif succ_walks[nodes] >= self.max_succ_walks:
                    # print(f"{max_succ_walks} successful walks, finishing run")
                    break
    ...
    def run_path(self, node_list, join_graph,
        ...
        for i, node in enumerate(node_list):
            ...
            if i == 0:
                assert node in self.init_sels, f"Node {node} not in init_sels"
                assert len(self.init_sels[node]) > 0, f"Node {node} has no init_sels"
