...
from datasets import datasets_list_dict
...
def create_db_statistics(dbms: str, dbms_kwargs: dict, col_stats_dir: str, target: str):
    statistics_dict = {}

    for dataset in datasets_list_dict['dataset_list_all']:
        dbms_wrapper = DBMSWrapper(dbms=dbms, dbms_kwargs=dbms_kwargs, db_name=dataset)
        ...
        col_stats_path = os.path.join(col_stats_dir, f'{dataset}_column_statistics.json')
        ...
        for entry in dataset_info:
            ...
            entry['PK'] = '1' if (dataset, table, column) in is_pk_list else '0'
            entry['use'] = 'NO' if (dataset, table, column) in skip_col_list else 'YES'
        ...
        statistics_dict[dataset] = dataset_info
