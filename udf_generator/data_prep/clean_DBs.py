...
from data_prep.utilities import db_to_dict


def create_SQL_file(db_name, table_col_map):
    """
    ...
    file = open("./data_prep/cleanDB_scripts/" + db_name + "_clean_up.sql", "w")
...
def perform_clean_up(db_name, tables, dbms: str, dbms_kwargs: dict):
    """
    ...
    if dbms == 'postgres':
        ...
        import duckdb
        conn = duckdb.connect(database=f'{dbms_kwargs["dir"]}/{db_name}.db')
        cur = conn
    else:
        raise NotImplementedError(f'Unknown dbms: {dbms}')

    file = "data_prep/cleanDB_scripts/" + db_name + "_clean_up.sql"
...
def clean_up_DBs(db: str, dbms: str, dbms_kwargs: dict, meta_data_path: str, nan_threshold=0.2, ):
    """
    ...
    try:
        ...
        db_dict = db_to_dict(db, 1, nan_threshold, meta_data_path)
        create_SQL_file(db, db_dict)
        card_after = perform_clean_up(db, db_dict.keys(), dbms=dbms, dbms_kwargs=dbms_kwargs)
    except Exception as e:
        print(db)
        print(e)
        raise e
