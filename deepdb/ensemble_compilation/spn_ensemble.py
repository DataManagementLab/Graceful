...
class RenameUnpickler(pickle.Unpickler):
    # rename classes to deepdb, if they have are created with old file structure
    def find_class(self, module, name):
        renamed_module = module
        if module.startswith("ensemble_compilation") or module.startswith('aqp_spn') or module.startswith('rspn'):
            renamed_module = f"deepdb.{module}"
        elif module.startswith('data_driven_cardinalities'):
            renamed_module = module.replace('data_driven_cardinalities.', '')
...
def read_ensemble(ensemble_locations, build_reverse_dict=False):
    ...
    for ensemble_location in ensemble_locations:
        with open(ensemble_location, 'rb') as handle:
            current_ensemble = RenameUnpickler(handle).load()
...
def recursively_get_spn_num_nodes(mspn):
    if hasattr(mspn, 'children'):
        sp_ctr = 1
        leaf_ctr = 0
        max_depth = 0
        for child in mspn.children:
            sp, leaf, depth = recursively_get_spn_num_nodes(child)
            sp_ctr += sp
            leaf_ctr += leaf
            if depth > max_depth:
                max_depth = depth
        return sp_ctr, leaf_ctr, max_depth + 1
    else:
        return 0, 1, 1
...
def evaluate_factors(dry_run, factors_full, cached_expecation_vals, confidence_intervals=False,
    ...
    evaluated_leaf_nodes = 0
    evaluated_sp_nodes = 0
    depth_sum = 0
    num_spn = 0
    cached = 0
        ...
        if not dry_run:
            if isinstance(factor, IndicatorExpectation):
                if cached_expecation_vals.get(hash(factor)) is not None:
                    ...
                    ctr1, ctr2, depth = recursively_get_spn_num_nodes(factor.spn.mspn)
                    evaluated_leaf_nodes += ctr2
                    evaluated_sp_nodes += ctr1
                    depth_sum += depth
                    num_spn += 1
    ...
    # assert num_spn+cached > 0, f'No SPN evaluated. Check if SPN is empty or not evaluated.\n Num factors: {len(factors)}'
    ...
    if confidence_intervals:
        ...
        cardinality_stds = std_of_products(factor_exps, factor_stds)
        return cardinality_stds, values, cardinality, formula, evaluated_leaf_nodes, evaluated_sp_nodes, depth_sum / num_spn if num_spn > 0 else 0, num_spn
    else:
        return values, cardinality, formula, evaluated_leaf_nodes, evaluated_sp_nodes, depth_sum / num_spn if num_spn > 0 else 0, num_spn
...
class SPNEnsemble:
    ...
    def evaluate_query(self, query, rdc_spn_selection=False, pairwise_rdc_path=None,
        ...
        if query.query_type == QueryType.CARDINALITY or any(
            ...
            _, factors, cardinalities, factor_values, evaluate_leaf_nodes, evaluated_sp_nodes, avg_max_depth, num_spns = self.cardinality(
                prototype_query,
            ...
            if len(query.group_bys) == 0 and confidence_intervals:
                _, factors_no_overlap, _, _, evaluate_leaf_nodes, evaluated_sp_nodes, avg_max_depth, num_spns = self.cardinality(
                    prototype_query,
                    rdc_spn_selection=rdc_spn_selection,
                    pairwise_rdc_path=pairwise_rdc_path,
                    dry_run=False,
            ...
            if len(query.group_bys) > 0:
                ...
                if confidence_intervals:
                    _, factors_no_overlap, _, factor_values_no_overlap, evaluate_leaf_nodes, evaluated_sp_nodes, avg_max_depth, num_spns = self.cardinality(
                        prototype_query,
                        rdc_spn_selection=rdc_spn_selection,
                        pairwise_rdc_path=pairwise_rdc_path,
                        dry_run=False,
    ...
    def _cardinality_with_injected_start(self, query, first_spn, next_mergeable_relationships, next_mergeable_tables,
        ...
        values, cardinality, formula, evaluated_leaf_nodes, evaluated_sp_nodes, avg_max_depth, num_spns = evaluate_factors(
            dry_run, factors, self.cached_expecation_vals,
            gen_code_stats=gen_code_stats, join_keys=join_keys, scale=scale)

        if not return_factor_values:
            return formula, factors, cardinality, evaluated_leaf_nodes, evaluated_sp_nodes, avg_max_depth, num_spns
        else:
            return formula, factors, cardinality, values, evaluated_leaf_nodes, evaluated_sp_nodes, avg_max_depth, num_spns
    ...
    def _merged_tables(self, mergeable_relationships):
        ...
        for relationship in mergeable_relationships:
            try:
                ...
                print(self.schema_graph.relationship_dictionary.keys())
                raise e
