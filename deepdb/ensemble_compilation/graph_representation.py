...
class UnexpectedJoinCondError(Exception):
...
class Query:
    ...
    def add_join_condition(self, relationship_identifier):
        # print(f'Adding join condition {relationship_identifier} \n{self.schema_graph.relationship_dictionary}')
        if relationship_identifier not in self.schema_graph.relationship_dictionary:
            reverse_relationship_identifier = relationship_identifier.split(' = ')[1] + ' = ' + \
                                              relationship_identifier.split(' = ')[0]
            if reverse_relationship_identifier in self.schema_graph.relationship_dictionary:
                relationship_identifier = reverse_relationship_identifier
            else:
                raise UnexpectedJoinCondError(
                    f"Relationship {relationship_identifier} not found in schema graph.\n{self.schema_graph.relationship_dictionary.keys()}")
