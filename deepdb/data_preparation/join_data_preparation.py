...
class JoinDataPreparator:
    ...
    def _get_table_data(self, path, table):
        ...
        if self.cached_tables.get(path) is not None:
            print(f'Return from cache {table}', flush=True)
            return self.cached_tables[path]

        table_data = pd.read_hdf(path, key='df')
        # print(f'Reading table {table} from {path}: {table_data.columns}',flush=True)
        ...
        if len(del_irr_attr) > 0:
            print(f'Dropping irrelevant attributes {del_irr_attr} from table {table}')
            ...
            while len(todo_relationships) > 0:
                ...
                # incoming edge, e.g. date joined, join lineorders
                    ...
                    try:
                        ...
                        logging.error(f"Could not find attribute {right_attribute} in table {next_joined_table}")
                        logging.error(next_table_data.columns)
                        raise e
                    # fix pandas error
                    df_samples.index.name = None
