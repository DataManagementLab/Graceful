...
def read_table_csv(table_obj, **read_kwargs):
    ...
    if len(table_obj.irrelevant_attributes) > 0:
        print(f'Drop columns: {table_obj.irrelevant_attributes} from {table_obj.table_name}', flush=True)
...
def prepare_single_table(schema_graph, table, path, read_kwargs, max_distinct_vals=10000, max_table_data=20000000):
    ...
    logger.info(f"Removed {len(cols_to_be_dropped)} columns due to functional dependencies ({cols_to_be_dropped})")
    ...
    logger.info(f'Table {table}: {table_data.columns}')
    for relationship_obj in incoming_relationships:
        ...
        assert table_data.index.isna().sum() == 0, "Index contains NaN values"
        ...
        logger.info(f"Median value {mu_col_name}: {np.median(muls[mu_col_name])}")
        logger.info(f"Mean value {mu_col_name}: {np.mean(muls[mu_col_name])}")
    ...
    del_cat_attributes = []
    logger.info('Columns: {}'.format(table_data.columns))
    ...
    for rel_attribute in relevant_attributes:
        ...
        # numerical value
            ...
            if not table_data[attribute].notna().any():
                ...
                # make sure that the artificial null-value is not otherwise seen in data
                if (table_data[attribute] == unique_null_val).any():
                    unique_null_val = table_data[attribute].mean() + 0.0002
