...
from deepdb.rspn.structure.leaves import IdentityNumericLeaf, identity_distinct_ranges, \
...
class AQPSPN(CombineSPN, RSPN):
    ...
    def _parse_conditions(self, conditions, group_by_columns=None, group_by_tuples=None, join_keys=None, scale=1):
        ...
        for (table, condition) in conditions:
            ...
            # revert changes in schema made for UDFs
            if table == 'canc' and condition.startswith(
                    'col_class') and f'{table}.class' in self.column_names:  # carcinogenesis
                condition = condition.replace('col_class', 'class')
            elif table == 'canc' and condition.startswith(
                    'col_class') and f'{table}.class_' in self.column_names:  # carcinogenesis
                condition = condition.replace('col_class', 'class_')
            elif table == 'canc' and condition.startswith('class') and not condition.startswith(
                    'class_') and f'{table}.class_' in self.column_names:  # carcinogenesis
                condition = condition.replace('class', 'class_')
            elif table in ['batting', 'teams', 'battingpost'] and condition.startswith(
                    'SecondB') and f'{table}.2B' in self.column_names:  # baseball
                condition = condition.replace('SecondB', '2B')
            elif table in ['batting', 'teams', 'battingpost'] and condition.startswith(
                    'ThirdB') and f'{table}.3B' in self.column_names:  # baseball
                condition = condition.replace('ThirdB', '3B')
            elif table == 'probehnuto_mimo_penezenku' and condition.startswith(
                    'Month_Year') and f'{table}.Month/Year' in self.column_names:  # seznam
                condition = condition.replace('Month_Year', 'Month/Year')
            ...
            matching_cols = []
            matching_fd_cols = []
            target_str = table + '.' + condition
            for op in ['<', '=', '>', ' ', '!=', ' IS NOT NULL']:
                matching_cols += [column for column in self.column_names if
                                  target_str.startswith(column + op) or target_str.endswith(op + column)]
                matching_fd_cols += [column for column in self.column_names if
                                     target_str.startswith(column + op) or target_str.endswith(op + column)]
            if len(matching_cols) == 0 and len(matching_fd_cols) == 0:
                for op in ['<', '=', '>', ' ', '!=', ' IS NOT NULL']:
                    matching_cols += [column for column in self.column_names if
                                      target_str.startswith(column.lower() + op) or target_str.endswith(
                                          op + column.lower())]
                    matching_fd_cols += [column for column in self.column_names if
                                         target_str.startswith(column.lower() + op) or target_str.endswith(
                                             op + column.lower())]
            ...
            assert len(matching_cols) == 1 or len(
                matching_fd_cols) == 1, f"Found multiple or no matching columns for table:{table}, condition: {condition}\n{matching_cols}\n{self.column_names}"
