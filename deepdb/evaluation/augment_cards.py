...
from deepdb.ensemble_compilation.graph_representation import Query, QueryType
from deepdb.ensemble_compilation.spn_ensemble import read_ensemble
from deepdb.utils.plan_utils import dumper, save_csv
...
def augment_bottom_up(schema, plan, q_id, database_statistics, spn_ensemble, q_stats, top_p, scale, non_inclusive=False,
    ...
    aggregation_below = 'Aggregate' in plan.plan_parameters.op_name or 'AGGREGATE' in plan.plan_parameters.op_name
    ...
    if t_idx is not None:
        ...
        elif hasattr(table_stats, 'table_name'):
            table_name = table_stats.table_name
        else:
            print(f'plan params: {plan.plan_parameters}\nt_idx: {t_idx}\n{table_stats}', flush=True)
    ...
    if filter_columns is not None:
        def augment_filter_recursively(fc):
            if len(fc.children) == 0 and fc.operator != 'AND':
                ...
                if c_idx is None:
                    # UDF filter detected
                    assert hasattr(fc, 'udf_name') and fc.udf_name is not None, f'expected an UDF filter: {fc}'
                    return
    ...
    # evaluate query
    act_card, est_card = get_act_est_card(plan.plan_parameters)
    ...
    # query not supported
    if not query_parsed:
        plan.plan_parameters.dd_est_card = est_card
    ...
    # we do not care about really small cardinalities
    elif (act_card is not None and est_card <= 1000 and act_card <= 1000):
        plan.plan_parameters.dd_est_card = est_card
        top_p.plan_parameters.est_pg += 1
        ...
        if plan.plan_parameters.op_name in {'Parallel Seq Scan', 'Hash Join', 'Nested Loop', 'Seq Scan', 'Materialize',
                ...
                or plan.plan_parameters.op_name in {'SEQ_SCAN', 'HASH_JOIN', 'NESTLOOP', 'FILTER'}:

            try:
                _, factors, cardinality_predict, factor_values, evaluate_leaf_nodes, evaluated_sp_nodes, avg_max_depth, num_spns = spn_ensemble \
        ...
        # ignore this in the stats since pg semantics for cardinalities are different for this operator
                                              ...
                                              'Sort', 'Parallel Index Scan', 'BitmapAnd', 'INDEX_SCAN'}:
            plan.plan_parameters.dd_est_card = est_card
            top_p.plan_parameters.est_pg += 1
        # copy the cardinality from the child
        elif plan.plan_parameters.op_name in {'PROJECTION'}:
            assert len(plan.children) == 1, f"Projection should have exactly one child: {len(plan.children)}"
            assert plan.plan_parameters.est_card == plan.children[
                0].plan_parameters.est_card, f'Projection should not change cardinality: {plan.plan_parameters.est_card} != {plan.children[0].plan_parameters.est_card}'
            plan.plan_parameters.dd_est_card = plan.children[0].plan_parameters.dd_est_card
            top_p.plan_parameters.est_deepdb += 1
        else:
...
def convert_query(schema, tables, filter_conditions, join_conditions, non_inclusive):
    ...
    if len(tables) > 1:
        if join_conditions is not None:
            # use custom defined join conditions
            for j in join_conditions:
                query.add_join_condition(j)
        else:
