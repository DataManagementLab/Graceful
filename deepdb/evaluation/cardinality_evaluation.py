...
from deepdb.db_connection.db_connection import DBConnection, TrueCardinalityEstimator
...
def evaluate_cardinalities(ensemble_location, physical_db_name, query_filename, target_csv_path, schema,
    ...
    latencies = []
    evaluated_leaf_nodes_list = []
    evaluated_sp_nodes_list = []
    avg_max_depth_list = []
    num_spns_list = []
    ...
    for query_no, query_str in enumerate(queries):
        ...
        logger.debug(f"\t\tEvaluated leaf nodes: {evaluate_leaf_nodes}")
        logger.debug(f"\t\tEvaluated sp nodes: {evaluated_sp_nodes}")
        logger.debug(f"\t\tAvg max depth: {avg_max_depth}")
        logger.debug(f"\t\tNum SPNs: {num_spns}")
        ...
        evaluated_leaf_nodes_list.append(evaluate_leaf_nodes)
        evaluated_sp_nodes_list.append(evaluated_sp_nodes)
        avg_max_depth_list.append(avg_max_depth)
        num_spns_list.append(num_spns)
    ...
    logger.info(f"Evaluated leaf nodes avg: {np.mean(evaluated_leaf_nodes_list):.2f}")
    logger.info(f"Evaluated sp nodes avg: {np.mean(evaluated_sp_nodes_list):.2f}")
    logger.info(f"Avg max depth avg: {np.mean(avg_max_depth_list):.2f}")
    logger.info(f"Num SPNs avg: {np.mean(num_spns_list):.2f}")
